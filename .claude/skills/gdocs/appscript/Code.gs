    ...
    switch (action) {
      ...
      default:
        return jsonResponse({
          ...
          message: 'Valid actions: getDocument, getDocumentByUrl, createDocument, updateDocument, appendContent, getComments, addComment, resolveComment, ping'
        });
    }
  } catch (err) {
    if (err.isAppError) {
...
/**
 * Extract document content into a structured response.
 */
