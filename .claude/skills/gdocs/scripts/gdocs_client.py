...
# __file__ = .claude/skills/gdocs/scripts/gdocs_client.py
...
def get_session() -> tuple:
    ...
    """
    url = _load_credential('GDOCS_APPSCRIPT_URL')
    key = _load_credential('GDOCS_APPSCRIPT_KEY')

    if not url:
        raise FileNotFoundError(
            "GDOCS_APPSCRIPT_URL not found in ~/.fe-skills/.env. Run /gdocs-setup first."
        )
    if not key:
        raise FileNotFoundError(
            "GDOCS_APPSCRIPT_KEY not found in ~/.fe-skills/.env. Run /gdocs-setup first."
        )
