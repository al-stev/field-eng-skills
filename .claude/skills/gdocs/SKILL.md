---
name: gdocs
description: "Use when reading or writing Google Docs — creating documents, updating content, appending text, or managing comments. Activate for Google Doc links or document collaboration tasks."
allowed-tools: Bash(uv run --project .claude/skills/gdocs python .claude/skills/gdocs/scripts/*.py *), Bash(./scripts/gmail-cdp-fetch.sh *)
...
Read and write access to Google Docs for fetching, creating, updating, and appending to documents. Uses a Google Apps Script web app as the backend, with requests routed through the Chrome debug instance (CDP) for Okta SSO authentication.
