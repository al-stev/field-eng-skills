...
def get_session() -> tuple:
    ...
    if not url:
        raise FileNotFoundError(
            "GCALENDAR_APPSCRIPT_URL not found in ~/.fe-skills/.env. Run /gcalendar-setup first."
        )
    if not key:
        raise FileNotFoundError(
            "GCALENDAR_APPSCRIPT_KEY not found in ~/.fe-skills/.env. Run /gcalendar-setup first."
        )
