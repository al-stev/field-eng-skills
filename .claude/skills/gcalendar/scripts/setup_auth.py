#!/usr/bin/env python3
"""
Google Calendar Apps Script setup helper.
...
def main():
    parser = argparse.ArgumentParser(
        description='Save Google Calendar Apps Script credentials and verify connectivity'
    )
    parser.add_argument(
        '--url',
        required=True,
    ...
    _save_credential('GCALENDAR_APPSCRIPT_URL', args.url)
    _save_credential('GCALENDAR_APPSCRIPT_KEY', args.key)
    ...
    test_url = f"{args.url}?{urlencode({'key': args.key, 'action': 'listCalendars'})}"
    ...
    try:
        ...
        print(json.dumps({
            "ok": True,
            "message": "Google Calendar Apps Script setup successful",
            "env_path": str(TSM_ENV),
