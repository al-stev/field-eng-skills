#!/usr/bin/env python3
"""
Gmail Apps Script setup helper.
...
def main():
    parser = argparse.ArgumentParser(
        description='Save Gmail Apps Script credentials and verify connectivity'
    )
    parser.add_argument(
        '--url',
        required=True,
    ...
    _save_credential('GMAIL_APPSCRIPT_URL', args.url)
    _save_credential('GMAIL_APPSCRIPT_KEY', args.key)
    ...
    test_url = f"{args.url}?{urlencode({'key': args.key, 'action': 'listLabels'})}"
    ...
    try:
        ...
        print(json.dumps({
            "ok": True,
            "message": "Gmail Apps Script setup successful",
            "env_path": str(TSM_ENV),
