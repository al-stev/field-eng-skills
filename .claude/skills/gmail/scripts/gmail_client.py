#!/usr/bin/env python3
"""
Gmail Apps Script client with CDP-based authentication.
...
def get_session() -> tuple:
    ...
    """
    url = _load_credential('GMAIL_APPSCRIPT_URL')
    key = _load_credential('GMAIL_APPSCRIPT_KEY')
...
def api_call(action: str, **params) -> dict:
    """
    Make an authenticated request to the Gmail Apps Script web app via CDP.

    Routes the request through the Chrome debug instance so Okta SSO
    is handled transparently by the browser.

    Args:
        action: The action to perform (search, getMessage, getThread, etc.)
    ...
    if not data.get('ok', False):
        ...
        raise Exception(f"Gmail Apps Script error: {error} - {message}")
